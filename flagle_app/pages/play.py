...
from src.play_utils import get_union_flag, combine_union_flags, np_flag_to_img, select_random_flag, get_blank_flag, reset_game, get_flag_image, country_info
...
with col1:
    ...
    with st.container():
        ...
        if 'selected_country' in st.session_state.keys() and st.session_state.selected_country != 'Select a Country':
            ...
            guess_union_flag = get_union_flag(st.session_state.name, st.session_state.selected_country)
            new_union_flag = combine_union_flags(st.session_state['union_flag'], guess_union_flag)
            ...
            if st.session_state.selected_country.strip().lower() == st.session_state['name'].strip().lower():
                ...
                country_info(st.session_state.name)
                st.button('PLAY AGAIN', key='play_again')

            st.session_state['guesses_union_flag'][st.session_state.guess_counter] = guess_union_flag
...
with col2:
    with st.container():
        ...
        for i in range(MAX_GUESS):
            st.subheader('{}.   {}'.format(i+1, st.session_state['guesses'][i]))
            if st.session_state.guesses[i] != '':
                st.image([get_flag_image(st.session_state['guesses'][i]), np_flag_to_img(st.session_state['guesses_union_flag'][i])], width=160)